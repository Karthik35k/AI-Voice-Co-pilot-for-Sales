...
const WORD_AMOUNT_PHRASES: [string, number][] = [
  ['one lakh', 100000],
  ['one lac', 100000],
...
export function calculatePayIn3(totalAmount: number) {
...
export function buildEmiResponse(totalAmount: number): string {
...
function asksHowItWorks(text: string): boolean {
  return (
...
function asksAboutFees(text: string): boolean {
  return (
...
function asksAboutVerification(text: string): boolean {
  return (
    /\bverify\b/.test(text) ||
...
function wantsToProceed(text: string): boolean {
  return (
...
function asksAboutEligibility(text: string): boolean {
  return (
    /\beligib/.test(text) ||
...
function mentionsPurchaseNeed(text: string): boolean {
  return (
...
export function matchCustomerIntent(userMessage: string): IntentMatch {
...
export function generateRuleBasedResponse(userMessage: string): string {
